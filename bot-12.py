...
def make_receipt_pdf(
    ...
    pdf.ln()
    pdf.set_font(face, "", 10)
    vals = [date_h, heb(pay), heb("תשלום עבור שירות"), f"{amount:.2f}"]
    for w, v in zip(col_w, vals):
        pdf.cell(w, 8, str(v), border=1, align="C")
    pdf.ln()
    pdf.set_font(face, "B", 14)
    total_he = 'סה"כ'[::-1]
    pdf.cell(0, 10, f"{amount:.2f} ₪   {total_he}", ln=True, align="R")
    pdf.ln(10)
    pdf.set_font(face, "", 8)
    pdf.cell(0, 5, heb("עוסק פטור ללא מעמ"), ln=True, align="C")
